...
void loop() {
  ...
  for (int i = 0; i < totalFloors; i++) {
    ...
    if (!arrived) {
      ...
      //As long as the elevator moves we flicker the LED with an interval of "flickerTime" staying ON and OFF
      if (flickeringLedState == HIGH && (millis() - lastFlicker) > flickerTime) {
        flickeringLedState = LOW;
        digitalWrite(flickeringLed, flickeringLedState);
        lastFlicker = millis();
      }

      if (flickeringLedState == LOW && (millis() - lastFlicker) > flickerTime) {
